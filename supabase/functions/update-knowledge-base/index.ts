...
async function deleteFileFromGCP(filePath: string, accessToken: string) {
  const deleteUrl = `https://storage.googleapis.com/storage/v1/b/${BUCKET_NAME}/o/${encodeURIComponent(filePath)}`;
...
serve(async (req) => {
  ...
  try {
    ...
    if (!knowledgeBaseId || !name) {
      throw new Error("Knowledge base ID and name are required");
    }

    // Get GCP access token
    const accessToken = await getGCPAccessToken();

    // Delete files from GCP and database
    if (filesToDelete.length > 0) {
      for (const fileId of filesToDelete) {
        // Get file info from database
        const { data: fileData } = await supabase
          .from("knowledge_base")
          .select("gcp_file_path")
          .eq("id", fileId)
          .eq("user_id", user.id)
          .single();

        if (fileData?.gcp_file_path) {
          // Delete from GCP
          await deleteFileFromGCP(fileData.gcp_file_path, accessToken);
        }
    ...
    for (const file of newFiles) {
      if (file.size > 0) {
        const publicUrl = await uploadFileToGCP(file, knowledgeBaseId, accessToken);
        const gcpFilePath = `${knowledgeBaseId}/${file.name}`;
    ...
    // Update main knowledge base record
